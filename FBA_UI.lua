...
-- Requires FBA_Core.lua, FBA_Spells.lua
...
btn:SetScript("OnDragStop", function(self)
  ...
  -- snap around minimap edge by angle
...
cbEnabled:SetScript("OnClick", function(self)
  FBA.db.enabled = self:GetChecked()
end)
...
cbSplash:SetScript("OnClick", function(self)
  FBA.db.showAlert = self:GetChecked()
end)
...
-- Page buttons (simple paging instead of a scroll template)
...
-- Right panel: details for selected tracked buff
...
cbSpellEnabled:SetScript("OnClick", function(self)
  ...
  if key and FBA.db.spells[key] then
...
ddSound:SetScript("OnClick", function(self)
  -- cycle default -> none -> custom -> default
  local m = self._mode
...
ebSound:SetScript("OnTextChanged", function(self)
  local key = FBA.UI_selectedKey
  if key and FBA.db.spells[key] and FBA_DDSound._mode == "custom" then
...
btnTest:SetScript("OnClick", function()
  ...
  if key and FBA.db.spells[key] then
    ...
    -- play sound + show splash preview
...
function FBA:UI_SwitchTab(which)
  ...
  if which == "tracked" then
    ...
    FBA_BookFilter:Hide()
    -- show tracked rows, hide book rows
    for i=1,visibleRows do
      rows[i]:Show()
      bookRows[i]:Hide()
    end
...
function FBA:UI_Refresh()
  -- globals
  FBA_CBEnabled:SetChecked(FBA.db.enabled)
  FBA_CBSplash:SetChecked(FBA.db.showAlert)
  FBA_CBCountdown:SetChecked(FBA.db.alertCountdown)
  FBA_CBMinimap:SetChecked(FBA.db.minimap.show)
...
-- Public UI controls
function FBA:UI_Show() frame:Show(); FBA:UI_SwitchTab(FBA.UI_tab or "tracked") end
function FBA:UI_Hide() frame:Hide() end

function FBA:UI_Init()
  -- Hook delay EditBox to write-through
